...
class AuthState extends ChangeNotifier {
  ...
  // ── TODO 9 (DONE): Persist session to FlutterSecureStorage ─────────────
  ...
  /// Silently swaps the access token using the stored refresh token.
  /// Returns true if successful. Call this when a 401 is received.
  Future<bool> refreshAccessToken() async {
    final rt = _user?.refreshToken;
    if (rt == null) return false;
    try {
      final newToken = await _authService.refreshAccessToken(rt);
