    ...
    for seat in seats:
        booked = (
            ...
            .filter(
                models.Booking.event_id == event_id,
                models.Booking.status == BookingStatus.confirmed,
                models.BookingSeat.seat_id == seat.id,
            )
            .first()
        )
        if booked:
            ...
            locked = db.query(models.SeatLock).filter(
                models.SeatLock.event_id == event_id,
                models.SeatLock.seat_id == seat.id,
                models.SeatLock.status == SeatLockStatus.locked,
                models.SeatLock.expires_at > now,
            ).first()
...
def process_payment(db: Session, event_id: int, user_id: int, booking_id: int):
    """
